...
public class CommAppDAO implements CommAppDAO_interface {
	...
	private static final String GET_BY_BOOK = "SELECT comm_no, book_no, comm.mem_no, comm_desc, comm_level,"
			+ " to_char(comm_date, 'yyyy-MM-dd')comm_date, mem_name "
			+ "FROM comm left JOIN mem ON comm.mem_no = mem.mem_no WHERE book_no =? ORDER BY comm_no DESC";
	...
	@Override
	public Map<String, CommVO> getByBook(Integer book_no) {
		Map<String, CommVO> list = new LinkedHashMap<String, CommVO>();
		CommVO commVO = null;
		Connection con = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		...
		try {
			...
			while (rs.next()) {
				String comment_str = rs.getString("comm_no")+","+rs.getString("mem_name");
				commVO = new CommVO();
				commVO.setComm_no(rs.getInt("comm_no"));
				commVO.setBook_no(rs.getInt("book_no"));
				commVO.setMem_no(rs.getInt("mem_no"));
				...
				commVO.setComm_level(rs.getInt("comm_level"));
				commVO.setComm_date(rs.getDate("comm_date"));
				list.put(comment_str, commVO); // Store the row in the list
			}
