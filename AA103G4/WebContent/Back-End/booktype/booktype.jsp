...
<script>
	$(function() {
		if("${addBooktype}".length!=0){swal("新增完成", "已新增", "success");}
		if("${updateBooktype}".length!=0){swal("修改完成", "已修改", "success");}
		if("${deleteBooktype}".length!=0){swal("刪除完成", "已刪除", "error");}
		if("${deletefail}".length!=0){swal("刪除失敗", "此類別尚有書籍", "error");}
	});
</script>
			...
			<div class="col-xs-8 col-sm-8 col-md-9 col-lg-8 main">
				...
				<div role="tabpanel">
					...
					<ul class="nav nav-tabs" role="tablist">
						<li role="presentation" class="h3 ${param.action=='query'? 'active' :''}"><a href="#tab1" aria-controls="tab1" role="tab" data-toggle="tab">查詢所有類別</a></li>
						<li role="presentation" class="h3 ${param.action=='add'? 'active' :''}"><a href="#tab2" aria-controls="tab2" role="tab" data-toggle="tab">新增書籍類別</a></li>
						<li role="presentation" class="h3 ${param.action=='update'? 'active' :''}"><a href="#tab3" aria-controls="tab3" role="tab" data-toggle="tab">修改書籍類別</a></li>
					</ul>
					<!-- 標籤面板：內容區 -->
					<div class="tab-content">
						<br>
						<div role="tabpanel" class="tab-pane ${param.action=='query'? 'active' :''}" id="tab1">
							<jsp:include page="listBooktypes.jsp" />
						</div>
						<div role="tabpanel" class="tab-pane ${param.action=='add'? 'active' :''}" id="tab2">
							<jsp:include page="addBooktype.jsp"></jsp:include>
						</div>
						<div role="tabpanel" class="tab-pane ${param.action=='update'? 'active' :''}" id="tab3">
						<%
							if (request.getAttribute("updateBooktypeVO") != null) {
						%>
									<jsp:include page="updateBooktype.jsp" />
						<%
								}else{
						%>
						<div class="h2">
